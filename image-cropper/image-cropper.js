Component({
  ...
  data: {
    ...
    _img_top: my.getSystemInfoSync().windowHeight / 2, //图片上边距
    _img_left: my.getSystemInfoSync().windowWidth / 2, //图片左边距
    watch: {
      //监听截取框宽高变化
      width(value, that) {
  ...
  didMount() {
    this.data.info = my.getSystemInfoSync();
    //启用数据监听
    this._watcher();
  ...
  methods: {
    ...
    imgReset() {
      ...
      this.setData({
        scale: 1,
        angle: 0,
        _img_top: my.getSystemInfoSync().windowHeight / 2,
        _img_left: my.getSystemInfoSync().windowWidth / 2
      });
    },
    /**
     * 加载（更换）图片
    ...
    imageLoad(e) {
      setTimeout(() => {
        ...
        this.props.onLoadimage(this.data.imageObject);
      }, 1000);
    },
    /**
     * 设置图片放大缩小
    ...
    _initCanvas() {
      //初始化canvas
      if (!this.data.ctx) {
        this.data.ctx = my.createCanvasContext('image-cropper');

        // this.data.ctx.width = this.data._canvas_width * this.data.export_scale
        // this.data.ctx.height = this.data._canvas_height * this.data.export_scale
      }
    },
    /**
     * 根据开发者设置的图片目标尺寸计算实际尺寸
    ...
    _click(event) {
      ...
      this._draw(() => {
        let x = event.detail ? event.detail.pageX : event.touches[0].clientX;
        let y = event.detail ? event.detail.pageY : event.touches[0].clientY;
        if (
          x >= this.data.cut_left &&
          x <= this.data.cut_left + this.data.width &&
          (y >= this.data.cut_top && y <= this.data.cut_top + this.data.height)
        ) {
          //生成图片并回调
          //this.generateGraph(this.props.onClickcut)
        }
      });
    },
    //渲染
    ...
    _cutTouchMove(e) {
      if (this.data._flag_cut_touch && this.data.MOVE_THROTTLE_FLAG) {
        ...
        if(this.data.CUT_START.corner!=7&&this.data.CUT_START.corner!=8)
          height =
            this.data.CUT_START.height +
            ((this.data.CUT_START.corner > 1 && this.data.CUT_START.corner < 4)||this.data.CUT_START.corner==5//111111
              ? 1
              : -1) *
        ...
        switch (this.data.CUT_START.corner) {
          case 1:
            ...
            cut_left =
              this.data.CUT_START.cut_left -
              (width - this.data.CUT_START.width);
            break;
          case 2:
            width =
              this.data.CUT_START.width +
    ...
    _cutTouchStart(e) {
      ...
      //上中
      let cutbox_top_top = this.data.cut_top-30;
      let cutbox_bottom_top = this.data.cut_top+30;
      let cutbox_left_top = this.data.cut_left+this.data.width / 2 - 32;
      let cutbox_right_top = this.data.cut_left+this.data.width / 2 + 32;

      //下中
      let cutbox_top_bottom = this.data.cut_top+this.data.height-30;
      let cutbox_bottom_bottom = this.data.cut_top+this.data.height+30;
      let cutbox_left_bottom = this.data.cut_left+this.data.width / 2 - 32;
      let cutbox_right_bottom = this.data.cut_left+this.data.width / 2 + 32; 

      //左中
      let cutbox_top_left = this.data.cut_top+this.data.height/2-32;
      let cutbox_bottom_left = this.data.cut_top+this.data.height/2+32;
      let cutbox_left_left = this.data.cut_left - 30;
      let cutbox_right_left = this.data.cut_left + 30;

      //右中
      let cutbox_top_right = this.data.cut_top+this.data.height/2-32;
      let cutbox_bottom_right = this.data.cut_top+this.data.height/2+32;
      let cutbox_left_right = this.data.cut_left+this.data.width - 30;
      let cutbox_right_right = this.data.cut_left+this.data.width + 30;
      ...
      if (//右下角
        ...
        currentX>cutbox_left_top &&
        currentX<cutbox_right_top &&
        currentY>cutbox_top_top &&
        currentY<cutbox_bottom_top
      ){
        //console.log("上中")
        ...
        currentX>cutbox_left_bottom &&
        currentX<cutbox_right_bottom &&
        currentY>cutbox_top_bottom &&
        currentY<cutbox_bottom_bottom
      ){
        //console.log("下中")
        ...
        currentX>cutbox_left_left &&
        currentX<cutbox_right_left &&
        currentY>cutbox_top_left &&
        currentY<cutbox_bottom_left
      ){
        //console.log("左中")
        ...
        currentX>cutbox_left_right &&
        currentX<cutbox_right_right &&
        currentY>cutbox_top_right &&
        currentY<cutbox_bottom_right
      ){
        //console.log("右中")
        this._moveDuring();
        this.data._flag_cut_touch = true;
    ...
    _watcher() {
      Object.keys(this.props).forEach(v => {
        this._observe(this.props, v, this.data.watch[v]);
      });
    },
    ...
    handleConfirmTap(){
      this.generateGraph(this.props.onConfirm);
    },
